...
def wordworks(str, ri) -> int:
    ...
    if get == tDigit:
        if str[0] == rules[ri]:
            cached[ri][str] = 1
            return 1
        print(f"{globcurrent} fail for\n{gett(rtype[ri])}:{rules[ri]} {str}")
        return 0
        ...
        #print(f"3L{len(inspect.stack(0))}\t{str}")
        ...
        matchlen = wordworks(str,b)
        if matchlen != 0:
            matchlen2 = wordworks(str[matchlen:],c)
            if matchlen2 != 0:
                cached[ri][str] = matchlen+matchlen2
                return matchlen+matchlen2
        ...
        #print(f"5L{len(inspect.stack(0))}\t{str}")
        ...
        if matchlen != 0:
            matchlen2 = wordworks(str[matchlen:],d)
            if matchlen2 != 0:
                matchlen3 = wordworks(str[(matchlen+matchlen2):],e)
                if matchlen3 != 0:
                    cached[ri][str] = matchlen+matchlen2+matchlen3
                    return matchlen+matchlen2+matchlen3
